...
## Запуск чата из командной строки

Рядом с приложением собирается второй, консольный бинарник `llm-desktop-cli.exe`.
Он просит работающее приложение открыть новый чат и **ждёт ответа**, поэтому годится
для скриптов и для агента: код возврата 0 означает, что сессия действительно поднялась.

```sh
llm-desktop-cli --new-chat --folder "E:\Projects\Personal\app" --prompt "почини сборку"
```
...
Коды возврата: `0` — сессия запущена, `1` — ошибка (текст в stderr), `2` — неверные аргументы.

**Permission mode задать нельзя** — сессия всегда стартует с настройкой приложения по
умолчанию. Иначе любой, кто может выполнить команду, поднимал бы сессию с полным доступом
в обход владельца машины.

Приложение должно быть запущено: заявка кладётся файлом в
`%LOCALAPPDATA%\llm-desktop
equests\`, приложение её забирает и пишет туда же ответ.
Если приложение закрыто, команда завершится с ошибкой по таймауту, но сама заявка
останется и сработает при следующем запуске.

### Чтобы этим пользовался агент

Ничего регистрировать не нужно — ни MCP-сервера, ни скилла: и то и другое постоянно
занимало бы место в контексте модели. Просто скажите агенту команду в тот момент,
когда она нужна. Чтобы не писать полный путь каждый раз, добавьте папку с
`llm-desktop-cli.exe` в `PATH`.
